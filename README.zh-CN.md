...
- 支持 `.dsh` import graph 和 `DShader/Packages` 可复用包。
...
| `AutoCompileOnSave` | `true` | 保存 `.dsm` / `.dsh` 时刷新受影响资产。 |
...
DreamShader 现在可以在 `Graph` block 中编写 UE Substrate Front Material 图。声明 `Substrate` 输出，绑定到 `Base.FrontMaterial`，设置 `ShadingModel = "Substrate"` 或 `"Strata"`，然后用 `UE.Substrate*` 节点搭建 BSDF 树。
...
常用别名包括 `UE.SubstrateSlab`、`UE.SubstrateUnlit`、`UE.SubstrateHorizontalBlend`、`UE.SubstrateVerticalLayer`、`UE.SubstrateAdd`、`UE.SubstrateWeight` 和 `UE.SubstrateSelect`。也可以用 `UE.Expression(Class="SubstrateSlabBSDF", OutputType="Substrate", ...)` 反射创建 Substrate 表达式。`Substrate` 是 Graph-only 值：可以连接到 Substrate 表达式输入、`ShaderFunction` / `VirtualFunction` 的 Substrate pin，以及 `Base.FrontMaterial`，但不能从 Custom HLSL 节点返回，也不能参与普通数值运算。
...
- 更完整的 VSCode 语义诊断。
- 更完整的 Substrate utility 覆盖和高级模板。
